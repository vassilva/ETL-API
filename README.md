...
Detect source-control changes.
Retrieve the latest code from GitHub.
Read the version-controlled Jenkinsfile.
Execute the ETL process.
Connect to PostgreSQL using Jenkins-managed credentials.
Execute the automated Data Quality regression suite.
Fail the pipeline if ETL execution or automated validation fails.
Mark the pipeline as successful only when all validation steps pass.
...
Starts the Windows agent execution.
Retrieves the pipeline definition.
Executes the ETL process.
Extracts source datasets.
Applies transformation rules.
Loads target PostgreSQL tables.
Executes the automated Data Quality regression suite.
...
Pull Request workflow
CI Quality Gate
Power BI business validation
SQL-based KPI reconciliation
